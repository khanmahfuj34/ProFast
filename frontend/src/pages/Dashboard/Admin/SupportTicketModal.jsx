...
const SupportTicketModal = ({ ticket, isOpen, onClose }) => {
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/50 backdrop-blur-sm animate-in fade-in duration-200">
            <div className="bg-white w-full max-w-4xl max-h-[90vh] rounded-[32px] shadow-2xl flex flex-col overflow-hidden animate-in zoom-in-95 duration-200">
                {/* Header */}
                ...
                <div className="flex-1 overflow-y-auto flex flex-col md:flex-row">
                    ...
                    <div className="flex-1 p-8 border-r border-slate-100">
                        ...
                        <div className="mb-8">
                            <div className="flex items-center gap-3 mb-4 text-slate-400 text-sm">
                                ...
                                <div>
                                    <p className="font-bold text-slate-700">{ticket.userEmail}</p>
                                    <p className="text-xs">Reported on {new Date(ticket.createdAt).toLocaleString()}</p>
                                </div>
                            </div>
                            <div className="bg-slate-50 rounded-2xl p-6 text-slate-700 whitespace-pre-wrap font-medium">
                                {ticket.description}
                            </div>
                        </div>
                            ...
                            <div className="space-y-6 mb-8">
                                ...
                                {ticket.replies.map((reply, index) => (
                                    <div key={index} className="flex gap-4">
                                        <div className="w-8 h-8 rounded-full bg-lime-100 flex-shrink-0 flex items-center justify-center text-lime-600 font-bold shadow-sm">
                                            {reply.adminName?.charAt(0).toUpperCase() || 'A'}
                                        </div>
                                        <div className="flex-1 bg-white border border-slate-100 shadow-sm rounded-2xl p-5">
                                            <div className="flex items-center justify-between mb-2">
                                                <p className="font-bold text-slate-900">{reply.adminName || 'Admin Support'}</p>
                    ...
                    <div className="w-full md:w-80 bg-slate-50 p-6 flex flex-col gap-6">
                        ...
                        <div className="bg-white rounded-2xl p-5 border border-slate-100 shadow-sm">
                            <h4 className="text-xs font-black text-slate-400 uppercase tracking-widest mb-4">User Info</h4>
                            <div className="space-y-3">
                                <div className="flex items-center gap-3 text-sm text-slate-600 font-medium">
                                    <RiMailLine className="text-slate-400" /> {ticket.userEmail}
                                </div>
                                <div className="flex items-center gap-3 text-sm text-slate-600 font-medium">
                                    <RiTimeLine className="text-slate-400" /> {new Date(ticket.createdAt).toLocaleDateString()}
                                </div>
                            </div>
                        </div>
                        ...
                        {ticket.status !== 'closed' && (
                            <div className="bg-white rounded-2xl p-5 border border-slate-100 shadow-sm flex-1 flex flex-col">
                                <h4 className="text-xs font-black text-slate-400 uppercase tracking-widest mb-4">Admin Reply</h4>
                                <textarea
                                    value={replyText}
                                    onChange={(e) => setReplyText(e.target.value)}
                                    placeholder="Type your reply to the user..."
                                    className="w-full h-32 text-slate-900 bg-slate-50 border border-slate-200 rounded-xl p-3 text-sm focus:ring-2 focus:ring-lime-500 outline-none resize-none mb-3"
                                />
                                <button
                                    onClick={() => replyMutation.mutate()}
                                    disabled={!replyText.trim() || isReplying}
                                    className="w-full bg-slate-900 hover:bg-green-800 text-white font-bold py-3 rounded-xl flex items-center justify-center gap-2 transition-colors disabled:opacity-50"
                                >
                                ...
                                <h4 className="text-xs font-black text-slate-400 uppercase tracking-widest mb-3">Quick Actions</h4>
                                <div className="space-y-2">
                                    {ticket.status !== 'resolved' && (
                                        <button
                                            onClick={() => statusMutation.mutate('resolved')}
                                            className="w-full flex items-center justify-center gap-2 py-2.5 px-4 bg-lime-50 text-lime-700 hover:bg-lime-100 rounded-xl text-sm font-bold transition-colors"
                                        >
                                    ...
                                    <button
                                        ...
                                        className="w-full flex items-center justify-center gap-2 py-2.5 px-4 bg-red-50 text-red-700 hover:bg-red-100 rounded-xl text-sm font-bold transition-colors"
                                    >
                        ...
                        {ticket.status === 'closed' && (
                            <div className="bg-slate-100 rounded-2xl p-5 border border-slate-200 flex items-center justify-center text-slate-500 font-bold text-sm">
